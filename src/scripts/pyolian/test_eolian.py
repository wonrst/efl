...
class TestEolianUnit(unittest.TestCase):
    ...
    def test_children_listing(self):
        obj = eolian_db.object_by_name_get('Efl.Ui.Button')
        l = list(obj.unit.children)
        self.assertGreater(len(l), 5) # Efl.Object, Gfx, Clickable, etc.
...
class TestEolianClass(unittest.TestCase):
    def test_class(self):
        ...
        self.assertEqual([c.name for c in cls.inherits], ['Efl.Loop_Consumer'])
        self.assertEqual([c.name for c in cls.inherits_full], ['Efl.Loop_Consumer', 'Efl.Object'])
...
class TestEolianPart(unittest.TestCase):
    def test_part(self):
        ...
        self.assertEqual(part.class_.name, 'Efl.Ui.Popup_Part')
