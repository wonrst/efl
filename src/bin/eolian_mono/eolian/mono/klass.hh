...
struct klass
{
   ...
   bool generate_constructors(OutputIterator sink, attributes::klass_def const& cls, Context const& context) const
   {
     ...
     auto constructors = cls.get_all_constructors();
     ...
     if (!as_generator(
                     ...
                     << (root ? "this" : "base")  << "(nativeInherit.class_initializer, \"" << inherit_name << "\", " << name_helpers::klass_get_name(cls) <<  "(), typeof(" << inherit_name << "), parent, ref klass)\n"
                     << scope_tab << "{\n"
                     ...
                     << scope_tab << scope_tab << "register_event_proxies();\n"
                     << scope_tab << "}\n"
                 ).generate(sink, std::make_tuple(constructors, constructors), context))
         return false;
